...
class BaseDataLoader(torch.utils.data.Dataset):
    ...
    def custom_collate(batch):
        ...
        output = {}
        output['voxel_ev'] = voxel_ev  # spconv 张量
        output['seg_label_ev'] = torch.from_numpy(seg_label_batches).to(torch.int64)  # 明确为 long
        output['p2v_map'] = p2v_map  # IntTensor [Ne]，事件→体素 索引
        output['locs'] = locs_batches  # LongTensor [Ne,4]=[b,x,y,t]
